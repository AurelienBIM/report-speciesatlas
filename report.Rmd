...
-- *Generalized Linear Model (GLM)* : it is an adaptable generalization of the linear regression. It allows to connect the linear model to the response variable with a link function. That function is *logit* in the package. That model is very general and can suit to most of the case.

-- *Generalized Additive Model (GAM)* [9] : it is close to a GLM but it benefits the properties of additive models. The linear relationship between the response and predictor variables is replaced by non-linear *smooths* functions. GAM can suit other cases than  GLM, notably the ones where there's a few observations.
